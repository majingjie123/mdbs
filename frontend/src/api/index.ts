...
export const api = {
  ...
  updateConnection: (id: number, data: any) => http.put(`/connections/${id}`, { data }) as Promise<ApiResponse>,
  ...
  syncPlanList:(connId)=>http.get("/sync-plans/plans",{params:{conn_id:connId}}),syncPlanGet:(planId)=>http.get("/sync-plans/plans/"+planId),syncPlanCreate:(data)=>http.post("/sync-plans/plans",data),syncPlanUpdate:(planId,data)=>http.put("/sync-plans/plans/"+planId,data),syncPlanDelete:(planId)=>http.delete("/sync-plans/plans/"+planId),syncPlanToggle:(planId,enable)=>http.post("/sync-plans/plans/"+planId+"/toggle",{enable}),syncPlanRunNow:(planId)=>http.post("/sync-plans/plans/"+planId+"/run-now"),monitorStatus:(connId)=>http.get("/monitor/"+connId+"/status"),monitorPoolStats:()=>http.get("/monitor/pool-stats"),sqlTaskList:(connId)=>http.get("/sql-tasks/tasks",{params:{conn_id:connId}}),sqlTaskGet:(taskId)=>http.get("/sql-tasks/tasks/"+taskId),sqlTaskCreate:(data)=>http.post("/sql-tasks/tasks",data),sqlTaskUpdate:(taskId,data)=>http.put("/sql-tasks/tasks/"+taskId,data),sqlTaskDelete:(taskId)=>http.delete("/sql-tasks/tasks/"+taskId),sqlTaskToggle:(taskId,enable)=>http.post("/sql-tasks/tasks/"+taskId+"/toggle",{enable}),sqlTaskRunNow:(taskId)=>http.post("/sql-tasks/tasks/"+taskId+"/run-now"),monitorVariables:(connId)=>http.get("/monitor/"+connId+"/variables"),listSequences:(connId,database?,schema?)=>http.get("/sequences/"+connId,{params:{database:database||'',schema:schema||''}}),getSequenceDDL:(connId,seqName,database?,schema?)=>http.get("/sequences/"+connId+"/"+encodeURIComponent(seqName)+"/ddl",{params:{database:database||'',schema:schema||''}}),createSequence:(connId,data,database?,schema?)=>http.post("/sequences/"+connId,data,{params:{database:database||'',schema:schema||''}}),dropSequence:(connId,seqName,database?,schema?)=>http.delete("/sequences/"+connId+"/"+encodeURIComponent(seqName),{params:{database:database||'',schema:schema||''}}),backupPreview: (filename: string) =>
  ...
  // ── 数据生成器 ──────────────────────────────
  getDataGenColumns: (connId: number, tableName: string, database?: string, schema?: string) =>
    http.get(`/data-generator/${connId}/columns`, { params: { table_name: tableName, database: database || '', schema: schema || '' } }) as Promise<ApiResponse<any[]>>,
  dataGenPreview: (connId: number, params: { table_name: string; row_count: number; database?: string; schema?: string }) =>
    http.post(`/data-generator/${connId}/preview`, params) as Promise<ApiResponse<{ columns: any[]; preview_rows: any[][]; total: number }>>,
  dataGenInsert: (connId: number, params: { table_name: string; row_count: number; database?: string; schema?: string }) =>
    http.post(`/data-generator/${connId}/insert`, params) as Promise<ApiResponse<{ inserted: number }>>,
}
