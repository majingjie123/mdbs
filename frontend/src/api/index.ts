...
export const api = {
  ...
  syncPlanList:(connId)=>http.get("/sync-plans/plans",{params:{conn_id:connId}}),syncPlanGet:(planId)=>http.get("/sync-plans/plans/"+planId),syncPlanCreate:(data)=>http.post("/sync-plans/plans",data),syncPlanUpdate:(planId,data)=>http.put("/sync-plans/plans/"+planId,data),syncPlanDelete:(planId)=>http.delete("/sync-plans/plans/"+planId),syncPlanToggle:(planId,enable)=>http.post("/sync-plans/plans/"+planId+"/toggle",{enable}),syncPlanRunNow:(planId)=>http.post("/sync-plans/plans/"+planId+"/run-now"),monitorStatus:(connId)=>http.get("/monitor/"+connId+"/status"),monitorVariables:(connId)=>http.get("/monitor/"+connId+"/variables"),backupPreview: (filename: string) =>
