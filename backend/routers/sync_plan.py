...
router = APIRouter(prefix="/api/sync-plans", tags=["同步计划管理"])
...
def _scheduler_loop(storage: DBStorage):
    """调度循环：每 30 秒检查一次启用的同步计划"""
    while _scheduler_running:
        try:
            now = datetime.now()
            plans = storage.list_sync_plans()
            for plan in plans:
...
def _update_next_run(storage: DBStorage, plan: dict):
    ...
    with sqlite3.connect(storage._db_path) as conn:
        conn.execute(
            'UPDATE sync_plans SET next_run=? WHERE id=?',
...
def _execute_plan(storage: DBStorage, plan: dict):
    ...
    try:
        ...
        from ..routers.backup import SYNC_LOG_DIR, _ensure_sync_logs_dir
        _ensure_sync_logs_dir()
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_name = plan['database'].replace(" ", "_").replace("/", "_")
        safe_plan = plan['name'].replace(" ", "_").replace("/", "_")
        output_path = f"{SYNC_LOG_DIR}/backup_{safe_name}_{safe_plan}_{timestamp}.sql"
        ...
        if success:
            storage.update_sync_plan_run_time(plan['id'], 'success')
    except Exception:
        pass
...
def list_plans(
    ...
    """列出同步计划"""
    try:
        plans = storage.list_sync_plans(conn_id)
...
def get_plan(
    ...
    """获取单个同步计划"""
    try:
        plan = storage.get_sync_plan(plan_id)
        if not plan:
...
def create_plan(
    ...
    """创建同步计划"""
    try:
        plan_id = storage.save_sync_plan(body)
        _start_scheduler_if_needed(storage)
        return {"success": True, "message": "同步计划已创建", "data": {"id": plan_id}}
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def update_plan(
    ...
    try:
        body['id'] = plan_id
        storage.save_sync_plan(body)
        return {"success": True, "message": "同步计划已更新"}
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def delete_plan(
    ...
    """删除同步计划"""
    try:
        storage.delete_sync_plan(plan_id)
        return {"success": True, "message": "同步计划已删除"}
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def toggle_plan(
    ...
    try:
        ...
        import sqlite3
        with sqlite3.connect(storage._db_path) as conn:
            conn.execute('UPDATE sync_plans SET enabled=?, updated_at=? WHERE id=?',
                         (1 if enable else True, datetime.now().isoformat(timespec='seconds'), plan_id))
            conn.commit()
        return {"success": True, "message": "同步计划已" + ("启用" if enable else "禁用")}
    except Exception as e:
        return {"success": False, "message": str(e)}
